...
`modbus_rtu` 是基于 ModBusRTU 协议的通信模块，提供完整的 ModBusRTU 协议栈实现，支持主机和从机模式，包含常用功能码和 CRC16 校验。

### 核心特性

- ✅ **主机模式**：支持作为主站读取和写入从站数据
- ✅ **从机模式**：支持作为从站响应主站请求
- ✅ **常用功能码**：支持 03（读保持寄存器）、06（写单个寄存器）、10/16（写多个寄存器）
- ✅ **CRC16 校验**：完整的 CRC16 校验计算和验证
- ✅ **错误处理**：统一的错误码系统和异常响应处理
- ✅ **超时控制**：支持可配置的超时时间
- ✅ **多实例支持**：支持多个 UART 实例
...
- **传感器数据采集**：读取气体传感器、温度传感器等 ModBusRTU 设备数据
- **工业控制**：控制 ModBusRTU 从站设备（如变频器、PLC 等）
- **数据采集系统**：作为 ModBusRTU 从站，响应主站的数据采集请求
...
ModBusRTU_Status_t ModBusRTU_ReadHoldingRegisters(
    ...
    uint8_t slave_address,          // 从机地址（1-247）
    uint16_t start_address,         // 起始寄存器地址
    uint16_t register_count,        // 寄存器数量（1-125）
    uint16_t *data,                 // 接收数据缓冲区
    uint32_t timeout                // 超时时间（毫秒），0表示使用默认超时
);
```
...
if (status == ModBusRTU_OK) {
    // 读取成功，registers[0] 到 registers[9] 包含读取的数据
}
```
...
ModBusRTU_Status_t ModBusRTU_WriteMultipleRegisters(
    ...
    uint8_t slave_address,          // 从机地址（1-247）
    uint16_t start_address,         // 起始寄存器地址
    uint16_t register_count,        // 寄存器数量（1-123）
    const uint16_t *data,           // 要写入的数据
    uint32_t timeout                // 超时时间（毫秒），0表示使用默认超时
);
```
...
ModBusRTU_Status_t ModBusRTU_SlaveProcess(
    uint8_t *function_code,      // 接收到的功能码（输出）
    uint16_t *start_address,     // 接收到的起始地址（输出）
    uint16_t *register_count,    // 接收到的寄存器数量（输出）
    uint8_t *data,                // 接收/发送数据缓冲区
...
ModBusRTU_Status_t ModBusRTU_SlaveSendResponse(
    ...
    const uint8_t *data,            // 响应数据缓冲区
    uint16_t data_length,           // 数据长度（字节数）
    uint32_t timeout                // 超时时间（毫秒），0表示使用默认超时
);
```
...
ModBusRTU_Status_t ModBusRTU_SlaveSendException(
    ...
    uint8_t slave_address,          // 从机地址
    uint8_t function_code,          // 功能码
    ModBusRTU_ExceptionCode_t exception_code,  // 异常码
    uint32_t timeout                // 超时时间（毫秒），0表示使用默认超时
);
```
...
在 `System/config.h` 中启用 ModBusRTU 模块：

```c
#define CONFIG_MODULE_MODBUS_RTU_ENABLED  1   /**< ModBusRTU协议栈模块开关 */
```

### UART 配置

在 `BSP/board.h` 中配置 UART（ModBusRTU 使用 UART 进行通信）：

```c
#define UART_CONFIGS {                                                                                    \
    {USART1, GPIOA, GPIO_Pin_9, GPIOA, GPIO_Pin_10, 9600, USART_WordLength_8b, USART_StopBits_1, USART_Parity_No, 1}, /* UART1：PA9(TX), PA10(RX)，9600，8N1，启用 */ \
}
```

**注意**：ModBusRTU 通常使用 9600 波特率、8 数据位、1 停止位、无校验位（8N1）。
...
|--------|------|
| `ModBusRTU_OK` | 操作成功 |
| `ModBusRTU_ERROR_NULL_PTR` | 空指针错误 |
...
| `ModBusRTU_ERROR_INVALID_FUNCTION_CODE` | 无效功能码 |
| `ModBusRTU_ERROR_EXCEPTION` | 异常响应 |

## ⚠️ 注意事项

1. **UART 初始化**：使用 ModBusRTU 前，必须先初始化对应的 UART 实例
2. **超时设置**：根据实际通信环境设置合适的超时时间，建议 500ms 以上
3. **从机地址范围**：从机地址范围为 1-247，0 和 248-255 为保留地址
4. **寄存器数量限制**：
   - 读保持寄存器：最多 125 个寄存器
   - 写多个寄存器：最多 123 个寄存器
5. **CRC 校验**：所有帧都包含 CRC16 校验，自动验证
6. **异常响应**：从机可以发送异常响应，主机会自动识别并返回 `ModBusRTU_ERROR_EXCEPTION`
7. **从机模式**：从机模式使用轮询方式，需要周期性调用 `ModBusRTU_SlaveProcess()`

## 🔗 依赖关系

### 必需依赖

- **UART 驱动层**：通过 `uart.h` 接口调用 `UART_Transmit()` 和 `UART_Receive()`
- **Delay 模块**：用于超时处理（可选，如果使用阻塞延时）

### 不直接访问硬件

- 遵循项目解耦原则，中间件层不直接访问硬件
- 所有硬件操作通过驱动层接口完成
...
int main(void)
{
    ...
    if (status == ModBusRTU_OK) {
        /* 解析数据 */
        uint16_t gas_concentration = registers[1];  /* 寄存器1：当前气体浓度 */
        uint16_t temperature = registers[7];        /* 寄存器7：环境温度 */
        /* ... */
    }
