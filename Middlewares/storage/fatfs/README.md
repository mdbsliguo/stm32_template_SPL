...
### 核心特�?
1. **统一错误�?*：将FatFS的`FRESULT`错误码转换为项目统一的`FatFS_Status_t`错误�?2. **参数校验**：所有接口都进行参数校验，提高代码健壮�?3. **多卷支持**：支持SPI和SDIO两种接口的存储设�?4. **分区格式�?*：提供便捷的分区格式化接口，支持MBR分区表创建和FAT32格式�?5. **SD卡状态监�?*：自动检测SD卡状态，提供状态查询接�?6. **简化接�?*：封装FatFS底层API，提供更易用的接�?
### 设计目标

- **易用�?*：简化FatFS的使用，减少学习成本
- **统一�?*：统一错误码和接口风格
- **健壮�?*：参数校验和错误处理
- **可维护�?*：清晰的接口设计和文�?
## 🔧 依赖关系
...
- **磁盘I/O�?*：`diskio.h`, `diskio.c` - 磁盘I/O接口
...
```c
typedef enum {
    FATFS_VOLUME_SPI = 0,   /**< SPI接口卷（�?�?*/
    FATFS_VOLUME_SDIO = 1   /**< SDIO接口卷（�?�?*/
} FatFS_Volume_t;
```
...
typedef enum {
    FATFS_OK = 0,                           /**< 操作成功 */
    FATFS_ERROR_NOT_IMPLEMENTED = -99,      /**< 功能未实�?*/
    FATFS_ERROR_NULL_PTR = -1,              /**< 空指针错�?*/
    ...
    FATFS_ERROR_NOT_READY = -6,             /**< 磁盘未就�?*/
    FATFS_ERROR_NO_FILE = -7,               /**< 文件不存�?*/
    FATFS_ERROR_NO_PATH = -8,               /**< 路径不存�?*/
    FATFS_ERROR_INVALID_NAME = -9,          /**< 无效文件�?*/
    FATFS_ERROR_DENIED = -10,               /**< 访问被拒�?*/
    FATFS_ERROR_EXIST = -11,                /**< 文件已存�?*/
    FATFS_ERROR_INVALID_OBJECT = -12,       /**< 无效对象 */
    FATFS_ERROR_WRITE_PROTECTED = -13,      /**< 写保�?*/
    FATFS_ERROR_INVALID_DRIVE = -14,        /**< 无效驱动�?*/
    FATFS_ERROR_NOT_ENABLED = -15,          /**< 未启�?*/
    FATFS_ERROR_NO_FILESYSTEM = -16,        /**< 无文件系�?*/
...
**参数**�?- `volume`：卷类型（`FATFS_VOLUME_SPI`或`FATFS_VOLUME_SDIO`�?- `path`：卷路径（如`"0:"`或`"1:"`�?
**返回�?*�?- `FATFS_OK`：挂载成�?- `FATFS_ERROR_NO_FILESYSTEM`：无文件系统（需要格式化�?- 其他错误码：挂载失败
...
**参数**�?- `file`：文件对象指针（输出�?- `path`：文件路�?- `mode`：打开模式（`FA_READ`, `FA_WRITE`, `FA_OPEN_ALWAYS`, `FA_CREATE_ALWAYS`等）

**打开模式**�?- `FA_READ`：只�?- `FA_WRITE`：只�?- `FA_OPEN_EXISTING`：打开已存在的文件（不存在则失败）
...
**参数**�?- `file`：文件对象指�?- `ofs`：偏移量（从文件开头）

**示例**（定位到文件末尾）：
```c
FSIZE_t file_size = f_size(&file);
FatFS_FileSeek(&file, (uint32_t)file_size);  // 定位到文件末�?```
...
**返回�?*�?- `FATFS_OK`：创建成�?- `FATFS_ERROR_EXIST`：目录已存在（可以忽略）
...
**参数**�?- `volume`：卷类型
- `path`：卷路径
- `free_clusters`：空闲簇数（可为NULL�?- `total_clusters`：总簇数（可为NULL�?
**示例**�?```c
uint32_t free_clusters, total_clusters;
FatFS_Status_t status = FatFS_GetFreeSpace(FATFS_VOLUME_SPI, "0:", 
                                            &free_clusters, &total_clusters);
if (status == FATFS_OK) {
    // 计算空闲空间（需要知道簇大小�?    FATFS* fs;
    DWORD cluster_size = fs->csize * 512;  // 簇大小（字节�?    uint64_t free_bytes = (uint64_t)free_clusters * cluster_size;
}
```
...
```c
typedef struct {
    uint32_t reserved_area_sectors;    /**< 保留区扇区数，默�?047 */
    uint32_t mcu_direct_area_mb;      /**< MCU直接访问区大小MB�?=不使用，默认100 */
    uint32_t partition_start_sector;  /**< FAT32分区起始扇区�?=自动计算 */
...
#### 预设配置�?
```c
/* 标准混合存储方案：MBR + 保留�?1MB) + MCU�?+ FAT32分区 */
...
FatFS_PartitionConfig_t config = {
    .reserved_area_sectors = 2047,      // 保留�?047扇区（约1MB�?    .mcu_direct_area_mb = 100,          // MCU直接访问�?00MB
...
**示例**�?```c
// 创建标准布局：MBR + 保留�?1MB) + MCU�?100MB) + FAT32分区
...
typedef enum {
    ...
    FATFS_SD_STATUS_INITIALIZED = 3,     /**< SD卡已初始�?*/
    FATFS_SD_STATUS_READY = 4,           /**< SD卡就绪（已初始化且可用） */
    FATFS_SD_STATUS_ERROR = 5,           /**< SD卡错�?*/
...
**说明**�?- 每次调用时会自动检查SD卡状态并更新
- 状态检查有最小间隔控制（`FATFS_STATUS_CHECK_INTERVAL_MS`），避免过于频繁检�?- 返回当前状态码
...
**返回结构�?*�?```c
typedef struct {
    FatFS_SDCardStatus_t current_status;  /**< 当前状�?*/
    FatFS_SDCardStatus_t last_status;     /**< 上次状�?*/
    uint8_t status_changed;               /**< 状态是否变化（1=变化�?=未变化） */
...
if (info.status_changed) {
    // 状态发生变�?    if (info.current_status == FATFS_SD_STATUS_NOT_PRESENT) {
        // SD卡已拔出
...
if (status == FATFS_OK) {
    ...
    FatFS_FileSync(&file);  // 同步到磁�?    FatFS_FileClose(&file);
}
...
FatFS_Status_t status = FatFS_FormatStandard(FATFS_VOLUME_SPI, 100);
if (status == FATFS_OK) {
    // 格式化成�?    // 布局：MBR + 保留�?1MB) + MCU�?100MB) + FAT32分区
}
    ...
    while (1) {
        ...
        if (info.status_changed) {
            if (info.current_status == FATFS_SD_STATUS_NOT_PRESENT) {
                // SD卡已拔出，卸载文件系�?                FatFS_Unmount(FATFS_VOLUME_SPI);
            } else if (info.current_status == FATFS_SD_STATUS_READY) {
                // SD卡已插入并就绪，重新挂载
...
FatFS_Status_t InitFileSystem(void)
{
    ...
    if (status == FATFS_ERROR_NO_FILESYSTEM) {
        // 格式化标准混合存储方�?        status = FatFS_FormatStandard(FATFS_VOLUME_SPI, 100);
        if (status != FATFS_OK) {
            return status;
        }
...
- 挂载路径：`"0:"`（逻辑驱动�?�?- 文件路径：`"0:filename.txt"`（绝对路径）或`"filename.txt"`（相对路径，已挂载到`"0:"`�?- 目录路径：`"0:dirname"`（绝对路径）或`"dirname"`（相对路径）

### 3. 分区格式�?
- 需要启用`FF_MULTI_PARTITION`和`FF_USE_MKFS`
- 格式化会清空SD卡所有数�?- 格式化过程可能需要较长时间（取决于SD卡容量）
- 格式化前需要确保SD卡已初始�?
### 4. SD卡状态检�?
- 状态检查有最小间隔控制，避免过于频繁检查影响性能
- 状态检查需要TF_SPI模块支持（用于发送CMD13命令�?- 状态变化标志需要手动清�?
### 5. 错误处理
...
if (status != FATFS_OK) {
    ...
    if (status == FATFS_ERROR_NO_FILE) {
        // 文件不存�?    } else if (status == FATFS_ERROR_DISK_ERROR) {
        // 磁盘错误
    }
}
```
...
### 7. 多线程安�?
- FatFS本身不是线程安全�?- 如果需要在多线程环境中使用，需要添加互斥锁保护
...
**原因**：SD卡未格式化或分区表损�?
**解决方法**�?1. 检查SD卡是否已初始�?2. 调用`FatFS_FormatStandard()`或`FatFS_FormatFAT32Only()`进行格式�?3. 格式化后重新挂载
...
### Q3: 格式化失�?
**原因**�?- 未启用`FF_MULTI_PARTITION`或`FF_USE_MKFS`
- SD卡未初始�?- SD卡写保护
- 内存不足

**解决方法**�?1. 检查`ffconf.h`配置
2. 确保SD卡已初始�?3. 检查SD卡写保护开�?4. 增加工作缓冲区大�?
### Q4: SD卡状态检查不准确

**原因**�?- TF_SPI模块未初始化
- 状态检查间隔设置过�?- SD卡硬件问�?
**解决方法**�?1. 确保TF_SPI模块已初始化
