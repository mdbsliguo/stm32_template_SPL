...
- **核心目标**
  - 演示如何使用外部中断（EXTI）实现旋转编码器的计数功能，通过状态机方法读取两个通道的当前状态，根据状态变化模式判断旋转方�?
- **学习重点**�?  - EXTI外部中断的配置和使用方法（双通道配置�?  - EXTI中断回调函数的编写（两个通道的回调函数）
  - 状态机方法判断旋转方向的算法（读取GPIO状态，根据状态变化模式判断）
  - 中断中只做简单操作（读取状态、更新计数），复杂操作在主循环处�?  - 理解中断服务程序（ISR）已在Core层实现，案例只需写回调函�?- **应用场景**：适用于需要检测旋转方向和计数的场景，如音量调节、参数设置、位置检测等
...
**?? 重要提示**�?- PB0和PB1配置�?*上拉输入**（`GPIO_MODE_INPUT_PULLUP`），适用于开漏输出的编码�?- 如果编码器内部已有上拉，可以改为浮空输入（`GPIO_MODE_INPUT_FLOATING`�?- 编码器电源独立供电，不依赖STM32

**?? 重要提示**�?- 案例是独立工程，硬件配置在案例目录下�?`board.h` �?- 如果硬件引脚不同，直接修�?`Examples/EXTI/EXTI02_RotaryEncoder_Counter/board.h` 中的配置即可
- 本案例使�?*双边沿触�?*，检测所有边沿变�?- 使用状态机方法：在中断中读取GPIO状态，根据状态变化模式判断方�?
---
...
- `soft_i2c`：软件I2C模块（OLED依赖�?- `uart`：UART模块（串口调试）
- `debug`：Debug模块（printf重定向）
- `log`：Log模块（分级日志）
- `error_handler`：错误处理模�?- `delay`：延时模�?- `system_init`：系统初始化模块
...
1. **系统初始化阶�?*�?   - 调用 `System_Init()` 初始化系统基础功能
   ...
   - 设置EXTI中断回调函数（EncoderA_Callback和EncoderB_Callback�?   - 使能EXTI中断
...
3. **中断处理阶段**（自动触发）�?   - 旋转编码器旋转，产生边沿变化
   ...
   - EXTI模块调用用户回调函数（`EncoderA_Callback()` �?`EncoderB_Callback()`�?   - 回调函数中：
     - 读取两个通道的当前GPIO状�?     - 使用状态机方法判断状态变�?     - 根据状态变化模式判断方向并更新计数�?
### 关键方法

1. **EXTI初始�?*：使�?`EXTI_HW_Init()` 初始化EXTI�?   - 参数：EXTI线号、触发模式（上升�?下降�?双边沿）、模式（中断/事件�?   - 本案例使用双边沿触发（`EXTI_TRIGGER_RISING_FALLING`�?
2. **EXTI回调设置**：使�?`EXTI_SetCallback()` 设置中断回调函数
   - 回调函数在中断中调用，应尽可能简�?   - 只做简单操作（如记录状态、更新计数），复杂操作在主循环处�?
3. **EXTI使能**：使�?`EXTI_Enable()` 使能EXTI中断
   - 使能后，EXTI中断才会触发

4. **方向检测算�?*：使用状态机方法判断旋转方向
   - 在中断中读取两个通道的当前GPIO状�?   - 根据状态变化模式判断方向：
     - 正转：状态变�?`00 -> 01` �?计数�?1
     - 反转：状态变�?`00 -> 10` �?计数�?1
   - 每个步进只计数一次（避免重复计数�?   - 不依赖时间窗口，更可�?
### 数据流向�?
```mermaid
graph LR
    %% 输入
...
flowchart TD
    %% 初始化阶�?    subgraph INIT[初始化阶段]
        ...
        LOG_INIT --> OLED_INIT[OLED初始�?br/>OLED_Init]
        OLED_INIT --> EXTI0_INIT[EXTI0初始�?br/>EXTI_HW_Init<br/>PB0 双边沿触发]
        EXTI0_INIT --> EXTI1_INIT[EXTI1初始�?br/>EXTI_HW_Init<br/>PB1 双边沿触发]
    ...
    subgraph INT[中断处理阶段]
        direction TB
        ENCODER_ROTATE[编码器旋�?br/>通道A/B边沿变化]
        ...
        CALLBACK --> READ_GPIO[读取GPIO状�?br/>GPIO_ReadPin]
...
- **`EXTI_HW_Init()`**：初始化外部中断
  ...
  - 在本案例中用于初始化EXTI0和EXTI1，都使用双边沿触�?
- **`EXTI_SetCallback()`**：设置EXTI中断回调函数
  - 参数：EXTI线号、回调函数指针、用户数据指�?  - 回调函数在中断中调用，应尽可能简�?  - 在本案例中用于设置EncoderA_Callback和EncoderB_Callback
...
### 中断服务程序（ISR�?
**?? 重要说明**�?- EXTI的ISR已在 `Core/stm32f10x_it.c` 中实�?- 案例**不需�?*实现ISR，只需要写回调函数
...
### 方向检测算�?
**核心原理**�?- 旋转编码器两个通道（A和B）输出正交信号，旋转时产生相位差90度的方波
- 使用状态机方法：在中断中读取两个通道的当前状态，根据状态变化模式判断方�?
**状态变化模�?*�?- 正转：`00 -> 01 -> 11 -> 10 -> 00`（循环）
- 反转：`00 -> 10 -> 11 -> 01 -> 00`（循环）
- 状态编码：bit0=通道A(PB0)，bit1=通道B(PB1)

**实现方法**�?- 在中断回调函数中读取两个通道的当前GPIO状�?- 与上一次状态比较，判断状态变�?- 只在特定状态变化时计数（避免重复计数）�?  - 正转：只�?`00 -> 01` 时计�?1
  - 反转：只�?`00 -> 10` 时计�?1
- 每个步进只计数一次，确保计数准确

**优势**�?- 不依赖时间窗口，更可�?- 快速旋转时也能准确计数
- 自然过滤编码器抖�?
---
...
{
    /* 读取两个通道的当前状�?*/
    uint8_t state_a = GPIO_ReadPin(GPIOB, GPIO_Pin_0) ? 1 : 0;  /* 通道A状�?*/
    uint8_t state_b = GPIO_ReadPin(GPIOB, GPIO_Pin_1) ? 1 : 0;  /* 通道B状�?*/
...
while(1)
{
    ...
    if (g_counter != last_counter)
    {
        ...
        /* UART日志输出 */
        LOG_INFO("ENCODER", "计数�? %d, 方向: %s", g_counter, direction_str);
...
- 编码器通道A �?PB0
- 编码器通道B �?PB1
- 编码器VCC �?独立电源（不依赖STM32�?- 编码器GND �?GND
- OLED SCL �?PB8（可选）
- OLED SDA �?PB9（可选）
...
### 步骤7：观察效�?
- 旋转编码器时，计数器会根据方向加�?- OLED显示计数和方向（如果使用�?- UART输出详细日志
- LED闪烁反馈（每次计数时闪烁�?
---
...
**可能原因**�?- 编码器通道A和B接反
- 状态机判断逻辑需要调�?
**解决方法**�?1. 交换编码器通道A和B的连�?2. 或者修改代码中的状态机判断逻辑（交�?1�?1�?
### 问题4：计数不准确（每个步进计数多次）

**可能原因**�?- 编码器旋转一个步进会产生4个状态变化，如果每个状态变化都计数会导致计�?4

**解决方法**�?1. 检查代码是否只在特定状态变化时计数（如00->01�?0->10�?2. 确保每个步进只计数一�?
### 问题5：负数显示不正确

**可能原因**�?- 使用了`OLED_ShowNum()`（无符号数显示函数）

**解决方法**�?1. 使用`OLED_ShowSignedNum()`显示有符号数，支持负数显�?
---

## 💡 扩展练习

1. **修改时间窗口**�?   - 尝试不同的时间窗口值（3ms�?ms�?0ms�?0ms�?   - 观察对计数准确性的影响

2. **使用其他EXTI�?*�?   - 尝试使用其他EXTI线（如PA0/PA1�?   - 注意EXTI线号与GPIO引脚的对应关�?
3. **添加消抖功能**�?   - 在主循环中添加软件消抖逻辑
   - 使用定时器实现硬件消�?
4. **多编码器计数**�?   - 使用多个EXTI线连接多个编码器
   - 实现多通道计数

5. **方向指示**�?   - 使用LED或OLED显示当前旋转方向
   - 添加方向变化统计
...
2. **中断回调函数应简�?*�?   - 在中断回调中只做简单操作（记录状态、更新计数）
   - 不要执行复杂操作（OLED显示、延时等�?
3. **性能开销**�?   - EXTI0和EXTI1使用独立中断向量，开销很小（约0.14-0.28微秒�?   - 对于旋转编码器（低频中断），开销完全可以忽略

4. **硬件配置**�?   - 案例是独立工程，硬件配置在案例目录下�?`board.h` �?   - 如果硬件引脚不同，直接修�?`board.h` 中的配置即可

5. **方向检测原�?*�?   - 使用状态机方法：在中断中读取GPIO状态，根据状态变化模式判断方�?   - 只在特定状态变化时计数�?0->01�?0->10），确保每个步进只计数一�?   - 不依赖时间窗口，更可靠，快速旋转时也能准确计数
...
**测试内容**�?- [ ] EXTI初始化正�?- [ ] 中断触发正常（两个通道都能触发�?- [ ] 方向检测正常（正转+1，反�?1�?- [ ] 计数器更新正常（每个步进只计数一次）
- [ ] 负数显示正常（使用OLED_ShowSignedNum�?- [ ] OLED显示正常
- [ ] UART日志输出正常
- [ ] LED反馈正常

**注意事项**�?- PB0和PB1配置为上拉输入，适用于开漏输出的编码�?- 使用双边沿触发，检测所有边沿变�?- 使用状态机方法判断方向，在中断中读取GPIO状�?- 只在特定状态变化时计数�?0->01�?0->10），确保每个步进只计数一�?- 使用OLED_ShowSignedNum显示有符号数，支持负数显�?- 中断回调函数只做简单操作，符合设计原则
