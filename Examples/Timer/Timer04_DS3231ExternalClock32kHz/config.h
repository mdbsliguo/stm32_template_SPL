/**
 * @file config.h
 * @brief Timer04_DS3231ExternalClock32kHz案例专用配置（独立工程）
...
/* 驱动层模块 */
#define CONFIG_MODULE_GPIO_ENABLED          1   /**< GPIO模块开关 - 必须（UART、OLED、DS3231依赖） */
#define CONFIG_MODULE_UART_ENABLED         1   /**< UART模块开关 - 必须（串口调试，新项目必须） */
#define CONFIG_MODULE_LED_ENABLED          0   /**< LED模块开关 - 禁用（案例不使用） */
#define CONFIG_MODULE_OLED_ENABLED         1   /**< OLED模块开关 - 启用（规范要求，除案例1外） */
#define CONFIG_MODULE_I2C_ENABLED          0   /**< I2C模块开关 - 禁用（OLED和DS3231使用软I2C） */
#define CONFIG_MODULE_SOFT_I2C_ENABLED    1   /**< 软件I2C模块开关 - 启用（OLED和DS3231需要） */
#define CONFIG_MODULE_SPI_ENABLED          0   /**< SPI模块开关 - 禁用（案例不使用） */
#define CONFIG_MODULE_SOFT_SPI_ENABLED    0   /**< 软件SPI模块开关 - 禁用（案例不使用） */
#define CONFIG_MODULE_TIMER_ENABLED       0   /**< 定时器模块开关 - 禁用（案例直接使用SPL库） */
...
#define CONFIG_MODULE_DS3231_ENABLED     1   /**< DS3231模块开关 - 启用（案例核心功能） */

/* 系统层模块 */
#define CONFIG_MODULE_DELAY_ENABLED        1   /**< 延时模块开关 - 必须（案例使用） */
#define CONFIG_MODULE_BASE_TIMER_ENABLED   1   /**< TIM2时间基准模块开关 - 必须（delay依赖，案例使用TIM2_TimeBase_GetTick） */
#define CONFIG_MODULE_TIM_SW_ENABLED       0   /**< 软件定时器模块开关 - 禁用（案例不使用） */
