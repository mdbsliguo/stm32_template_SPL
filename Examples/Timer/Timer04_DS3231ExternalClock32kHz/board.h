/**
 * @file board.h
 * @brief 硬件配置头文件（Timer04_DS3231ExternalClock32kHz案例独立工程专用）
...
/* UART统一配置表 - Timer04案例配置（USART1，PA9/PA10，115200，8N1） */
#define UART_CONFIGS {                                                                                    \
...
/* TIM1 ETR引脚配置 - Timer04案例配置 */
/* 注意：TIM1的ETR引脚是PA12，用于接收DS3231的32K输出（32kHz时钟） */
/* 重要：在STM32F103C8T6中，TIM3没有ETR引脚，只有TIM1和TIM2有ETR引脚 */
/* TIM2的ETR引脚是PA0，TIM1的ETR引脚是PA12 */
/* ?? 警告：TIM2已被TIM2_TimeBase模块占用（系统时间基准），不能用于外部时钟！ */
/* 因此本案例使用TIM1，TIM1的ETR引脚是PA12（不是PA0！） */
#define TIM1_ETR_PORT  GPIOA
#define TIM1_ETR_PIN   GPIO_Pin_12  /* PA12 = TIM1_ETR */
...
/* 时钟管理配置 - 默认配置（Timer04不使用，但需要定义以避免编译错误） */
...
#define CLKM_ADAPTIVE_ENABLE 0         /* 禁用自动调频（Timer04不使用） */
#define CLKM_IDLE_HOOK_ENABLE 0        /* 禁用空闲钩子（Timer04不使用） */
