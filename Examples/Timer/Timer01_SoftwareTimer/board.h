/**
 * @file board.h
 * @brief 硬件配置头文件（Timer01_SoftwareTimer案例独立工程专用）
...
/* LED统一配置表 - Timer01案例配置 */
#define LED_CONFIGS {                                                    \
    {GPIOA, GPIO_Pin_1, Bit_RESET, 1}, /* LED1：PA1，低电平点亮，启用（定时器1控制） */ \
    {GPIOA, GPIO_Pin_2, Bit_RESET, 1}, /* LED2：PA2，低电平点亮，启用（定时器2控制） */ \
}
...
/* 时钟管理配置 - 默认配置（Timer01不使用，但需要定义以避免编译错误） */
...
#define CLKM_ADAPTIVE_ENABLE 0         /* 禁用自动调频（Timer01不使用） */
