 ...
 * @file main_example.c
 * @brief 案例 - 软件定时器示例
 * @example Examples/Timer/Timer01_TIM_sw/main_example.c
 * @status 待调试
 ...
 *
 * 硬件配置：
 * 在 Examples/Timer/Timer01_TIM_sw/board.h 中配置：
 * - LED_CONFIGS: LED配置表
 * - BUTTON_PORT/BUTTON_PIN: 按钮引脚定义
 * - OLED_I2C_CONFIG: OLED I2C配置
 *
 * 使用方法：
 * 1. 打开案例工程：双击 Examples/Timer/Timer01_TIM_sw/Examples.uvprojx
 * 2. 根据实际硬件修改 Examples/Timer/Timer01_TIM_sw/board.h 中的配置
 * 3. 编译运行（F7编译，F8下载）
 * 
 * 注意：本案例是独立工程，无需复制文件
 */
