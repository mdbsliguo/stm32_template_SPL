/**
 * @file main_example.c
 * @brief Timer03 - DS3231外部方波输出示例
 * @details 演示使用DS3231的SQW引脚输出1Hz方波作为TIM2定时器的外部时钟源
 ...
 *
 * 功能说明：
 * - 使用DS3231的SQW引脚输出1Hz方波
 * - 配置TIM2使用ETR引脚接收外部时钟（DS3231的SQW输出）
 * - 实时读取TIM2->CNT寄存器值，观察外部时钟驱动的计数效果
 ...
 *
 * 技术要点：
 * - DS3231配置方波输出为1Hz（DS3231_SetSquareWave）
 * - 配置PA0为输入模式（TIM2_ETR）
 * - TIM2配置为外部时钟模式2（TIM_ETRClockMode2Config）
 * - ETR引脚（PA0）接收DS3231的SQW输出作为时钟源
 * - 外部时钟频率：1Hz（每秒计数一次）
 */
...
/**
 * @brief 初始化TIM1硬件定时器（外部时钟源：DS3231方波输出）
 * @return error_code_t 错误码（ERROR_OK表示成功）
 * @note 配置PA12为浮空输入（TIM1_ETR功能）
 * @note 配置TIM1使用ETR引脚（PA12）作为外部时钟源
 * @note ETR引脚接收DS3231的SQW输出（1Hz方波）
 * @note PSC=0, ARR=999，外部时钟频率1Hz，每1秒计数一次
...
static error_code_t TIM1_ExternalClock_Init(void)
{
    ...
    uint16_t arr = 9;       /* 自动重装载值：0-9计数范围（每10秒溢出一次，便于观察） */
    /* 注意：如果改为arr=999，则每1000秒（16分40秒）溢出一次 */
    ...
    /* 使用浮空输入模式，因为DS3231的SQW输出是推挽输出，不需要上拉 */
...
static void UpdateOLEDDisplay(uint16_t count_value, uint32_t overflow_count, uint32_t run_time_ms)
{
    ...
    if (first_update)
    {
        /* 首次更新：显示标题和配置信息 */
        OLED_Clear();
        OLED_ShowString(1, 1, "Timer03 Demo");
        OLED_ShowString(2, 1, "DS3231 SQW 1Hz");
        first_update = false;
    }
    
    /* 第3行：显示当前计数值和进度百分比 */
    /* 格式：CNT:123 (12%) */
    percent = (count_value * 100) / 10;  /* ARR+1 = 10（arr=9时） */
...
int main(void)
{
    ...
    /* ========== 步骤6：输出初始化信息 ========== */
    LOG_INFO("MAIN", "=== Timer03 DS3231外部方波输出示例 ===");
    ...
    {
        OLED_Clear();
        OLED_ShowString(1, 1, "Timer03 Init");
    ...
    /* ========== 步骤10：配置DS3231方波输出为1Hz ========== */
    LOG_INFO("MAIN", "配置DS3231方波输出为1Hz");
    ds3231_status = DS3231_SetSquareWave(DS3231_SQW_1HZ, 1);
    if (ds3231_status != DS3231_OK)
    {
        LOG_ERROR("MAIN", "DS3231方波输出配置失败: %d", ds3231_status);
        ErrorHandler_Handle(ds3231_status, "DS3231");
        if (oled_status == OLED_OK)
        {
    ...
    LOG_INFO("MAIN", "DS3231方波输出已配置: 1Hz（SQW引脚）");
    LOG_INFO("MAIN", "硬件连接: DS3231 SQW引脚 -> TIM1 ETR引脚（PA12）");
    ...
    if (tim1_status == ERROR_OK)
    {
        ...
        LOG_INFO("MAIN", "TIM1配置: ETR外部时钟源（PA12），PSC=0, ARR=9");
        LOG_INFO("MAIN", "外部时钟频率: 1Hz（DS3231 SQW输出）");
        LOG_INFO("MAIN", "TIM1计数范围: 0-9（每1秒计数一次，每10秒溢出一次）");
    ...
    /* ========== 步骤12：主循环 ========== */
    LOG_INFO("MAIN", "=== 开始读取TIM1计数值（外部时钟：1Hz） ===");
    LOG_INFO("MAIN", "TIM1工作原理说明：");
    LOG_INFO("MAIN", "  - TIM1使用外部时钟源（DS3231 SQW输出，1Hz方波）");
    LOG_INFO("MAIN", "  - ETR引脚（PA12）接收DS3231的SQW输出作为时钟源");
    LOG_INFO("MAIN", "  - PA12已配置为输入模式（TIM1_ETR）");
    LOG_INFO("MAIN", "  - 外部时钟频率：1Hz（每秒一个上升沿）");
    LOG_INFO("MAIN", "  - TIM1每接收到一个上升沿，CNT寄存器加1");
    LOG_INFO("MAIN", "  - CNT从0计数到9，然后自动回到0（溢出）");
    LOG_INFO("MAIN", "  - 每1秒计数一次，每10秒完成一次0-9计数循环（溢出）");
    ...
    while (1)
    {
        ...
        /* 更新OLED显示（每200ms更新一次，1Hz时钟更新较慢） */
        if ((current_time - last_oled_update_time) >= 200)
        {
            UpdateOLEDDisplay(count_value, overflow_count, run_time_ms);
            last_oled_update_time = current_time;
        }
        
        /* UART输出详细日志（每2秒输出一次，1Hz时钟更新较慢） */
        if ((current_time - last_log_time) >= 2000)
        {
            uint8_t percent = (count_value * 100) / 1000;
            LOG_INFO("TIM1", "=== TIM1状态（外部时钟：1Hz） ===");
            LOG_INFO("TIM1", "当前计数值: %d / 9 (%d%%)", count_value, percent);
            LOG_INFO("TIM1", "溢出次数: %lu (每10秒溢出一次)", overflow_count);
            LOG_INFO("TIM1", "运行时间: %lu.%03lu 秒", run_time_ms / 1000, run_time_ms % 1000);
            LOG_INFO("TIM1", "外部时钟频率: 1Hz（DS3231 SQW输出）");
            LOG_INFO("TIM1", "计数速度: 每1秒计数一次，每10秒完成一次0-9计数循环（溢出）");
