/**
 * @file config.h
 * @brief Flash12专用配置（独立工程）
 * @details 只启用Flash12需要的模块，禁用其他模块以减小代码体积
 * @note Flash12 - LittleFS文件系统综合测试案例
 */
...
#define CONFIG_MODULE_LED_ENABLED           1   /**< LED模块开关 - 必须（Flash12使用） */
#define CONFIG_MODULE_OLED_ENABLED         1   /**< OLED模块开关 - 必须（Flash12使用，默认显示器） */
#define CONFIG_MODULE_SPI_ENABLED           1   /**< SPI模块开关 - 必须（W25Q使用SPI2） */
#define CONFIG_MODULE_W25Q_ENABLED          1   /**< W25Q模块开关 - 必须（Flash12使用） */
#define CONFIG_MODULE_UART_ENABLED          1   /**< UART模块开关 - 必须（Flash12使用，详细日志输出） */
...
/* Flash12专用配置 */
#define CONFIG_LITTLEFS_FORCE_FORMAT  1   /**< 强制格式化开关：1=每次启动都格式化（解决NOSPC问题），0=只在挂载失败时格式化 */
#define CONFIG_LITTLEFS_REFORMAT_BEFORE_TEST10  1   /**< 测试10前重新格式化：1=在测试10前重新格式化（解决NOSPC问题），0=不重新格式化 */

/* 系统层模块 */
#define CONFIG_MODULE_DELAY_ENABLED        1   /**< 延时模块开关 - 必须（Flash12使用） */
#define CONFIG_MODULE_BASE_TIMER_ENABLED   1   /**< 基时定时器模块开关 - 必须（delay依赖） */
#define CONFIG_MODULE_SYSTEM_INIT_ENABLED   1   /**< 系统初始化模块开关 - 必须（Flash12使用） */
#define CONFIG_MODULE_IWDG_ENABLED         0   /**< 独立看门狗模块开关 - 禁用（Flash12不使用） */

/* 通用模块 */
#define CONFIG_MODULE_ERROR_HANDLER_ENABLED 1   /**< 错误处理模块开关 - 必须（Flash12使用） */
#define CONFIG_MODULE_LOG_ENABLED           1   /**< 日志模块开关 - 必须（Flash12使用） */
#define CONFIG_MODULE_MODULE_CTRL_ENABLED   0   /**< 模块开关总控开关 - 禁用（Flash12不使用） */
