/**
 * @file board.h
 * @brief 硬件配置头文件（案例6独立工程专用）
 * @details 此文件包含案例6所需的硬件配置
 * 
...
/* LED统一配置表 - 案例6配置（只需要LED1） */
#define LED_CONFIGS {                                                                \
    {GPIOA, GPIO_Pin_1, Bit_RESET, 1}, /* LED1：PA1，低电平点亮，启用（错误提示） */ \
}
...
#define CLKM_ADAPTIVE_ENABLE 0         /* 禁用自动调频（案例6使用手动模式） */
#define CLKM_IDLE_HOOK_ENABLE 0        /* 禁用空闲钩子 */
