/**
 * @file board.h
 * @brief 硬件配置头文件（Basic05_PhotoresistorControlBuzzer案例独立工程专用）
...
/* LED统一配置表 - 案例不使用LED，但需要定义以避免编译错误 */
#define LED_CONFIGS {                                                            \
    {GPIOA, GPIO_Pin_1, Bit_RESET, 0}, /* LED1：PA1，低电平点亮，禁用（案例不使用） */ \
}
...
/* Buzzer统一配置表 - 案例配置（GPIO模式） */
/* 注意：三引脚有源蜂鸣器模块，VCC和GND已连接，控制引脚（中间引脚，SIG/IN）连接到PA3 */
#define BUZZER_CONFIGS {                                                                                    \
    {BUZZER_MODE_GPIO, GPIOA, GPIO_Pin_3, 0, 0, Bit_RESET, 1}, /* Buzzer1：GPIO模式，PA3（控制引脚），低电平有效，启用 */ \
}
