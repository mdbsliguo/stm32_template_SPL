/**
 * @file board.h
 * @brief 硬件配置头文件（OLED02独立工程专用）
 * @details 此文件包含OLED02案例所需的硬件配置
 * 
 * 注意：这是独立工程的board.h，包含案例所需的硬件配置
 * 
 * 硬件要求：
 * - LED1：PA1（系统状态指示）
 * - W25Q SPI Flash模块（SPI2接口，存储中文字库）
 *   - CS：PA11
 *   - SCK：PB13（SPI2_SCK）
 *   - MISO：PB14（SPI2_MISO）
 *   - MOSI：PB15（SPI2_MOSI）
 * - OLED显示屏（软件I2C接口，用于显示中文）
 *   - SCL：PB8（软件I2C）
 *   - SDA：PB9（软件I2C）
 * - UART1（用于详细日志输出）
 *   - TX：PA9
...
/* SPI统一配置表 - OLED02配置（SPI2，PB13/14/15，CS=PA11） */
...
/* OLED I2C配置 - OLED02配置（软件I2C，PB8/9） */
...
/* UART统一配置表 - OLED02配置（UART1，PA9/10，115200） */
#define UART_CONFIGS {                                                                                    \
