 ...
 * @file config.h
 * @brief 案例3专用配置（独立工程）
 * @details 只启用案例3需要的模块，禁用其他模块以减小代码体积
 */
...
#define CONFIG_MODULE_OLED_ENABLED         1   /**< OLED模块开关 - 必须（案例3使用） */
#define CONFIG_MODULE_SOFT_I2C_ENABLED   1   /**< 软件I2C模块开关 - 必须（OLED使用） */

/* 系统层模块 */
#define CONFIG_MODULE_DELAY_ENABLED        1   /**< 延时模块开关 - 必须（案例3使用） */
#define CONFIG_MODULE_BASE_TIMER_ENABLED   1   /**< 基时定时器模块开关 - 必须（delay依赖） */
#define CONFIG_MODULE_CLOCK_MANAGER_ENABLED 0   /**< 时钟管理模块开关 - 禁用（案例3不使用） */
#define CONFIG_MODULE_SYSTEM_INIT_ENABLED   1   /**< 系统初始化模块开关 - 必须（案例3使用） */
#define CONFIG_MODULE_IWDG_ENABLED         0   /**< 独立看门狗模块开关 - 禁用（案例3不使用） */

/* 通用模块 */
#define CONFIG_MODULE_ERROR_HANDLER_ENABLED 0   /**< 错误处理模块开关 - 禁用（案例3不使用） */
#define CONFIG_MODULE_LOG_ENABLED           0   /**< 日志模块开关 - 禁用（案例3不使用） */
#define CONFIG_MODULE_MODULE_CTRL_ENABLED   0   /**< 模块开关总控开关 - 禁用（案例3不使用） */
#define CONFIG_MODULE_SYSTEM_MONITOR_ENABLED 0  /**< 系统监控模块开关 - 禁用（案例3不使用） */
