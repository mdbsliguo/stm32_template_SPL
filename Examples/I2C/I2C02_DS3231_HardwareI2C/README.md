...
- **LED1**：连接到 `PA1`（系统状态指示）
- **DS3231实时时钟模块**（I2C接口）：
  - SCL：`PB10`（I2C2�?  - SDA：`PB11`（I2C2�?  - VCC�?.3V
  - GND：GND
...
- `i2c`：硬件I2C驱动模块（DS3231使用I2C2�?- `ds3231`：DS3231实时时钟驱动模块（核心）
- `gpio`：GPIO驱动模块（I2C依赖�?- `led`：LED驱动模块（状态指示）
- `delay`：延时模�?- `error_handler`：错误处理模�?- `log`：日志模块（调试输出�?
## 🚀 使用步骤
...
```c
#define I2C_CONFIGS {                                                                    \
    {I2C2, GPIOB, GPIO_Pin_10, GPIOB, GPIO_Pin_11, 100000, 0x00, 1}, /* I2C2：PB10(SCL), PB11(SDA)�?00kHz */ \
}
```
...
1. **初始化阶�?*
   - 系统初始�?   - 硬件I2C初始化（I2C2: PB10/11�?   - DS3231初始化（配置硬件I2C接口�?
2. **DS3231配置阶段**
...
- **硬件I2C接口**：使用STM32硬件I2C外设，速度快，资源占用�?- **DS3231配置流程**：按照标准流程配置DS3231，确保正常工�?- **时间格式转换**：处理DS3231的BCD格式和标准时间格式的转换
...
flowchart TD
    %% 初始化阶�?    subgraph INIT[初始化阶段]
        ...
        START --> I2C_INIT[硬件I2C初始�?br/>I2C_HW_Init<br/>I2C2 PB10/11]
        I2C_INIT --> DS3231_INIT[DS3231初始�?br/>DS3231_Init<br/>硬件I2C接口]
...
**必须**在SCL和SDA线上添加上拉电阻�?.7kΩ-10kΩ）到3.3V，否则I2C通信会失败�?
### 2. 初始化顺�?
...
- 如果检测到OSF标志，说明振荡器曾经停止过，时间可能不准�?- 首次使用或电池耗尽后，需要清除OSF标志并重新设置时�?
### 4. 时间格式

- DS3231使用24小时�?- 星期�?=Sunday, 2=Monday, ..., 7=Saturday
- 年份范围�?000-2099

### 5. 闹钟匹配模式

- `DS3231_ALARM_MODE_ONCE_PER_SECOND`：每秒触�?- `DS3231_ALARM_MODE_SECOND_MATCH`：秒匹配
- `DS3231_ALARM_MODE_MIN_SEC_MATCH`：分秒匹�?- `DS3231_ALARM_MODE_HOUR_MIN_SEC_MATCH`：时分秒匹配
...
- 如果配置为`DS3231_INT_MODE_ALARM`，方波输出会被禁�?
## 🔍 测试方法

### 测试1：基本通信测试

1. 编译并下载程�?2. 观察串口输出，应该能看到DS3231初始化成功的消息
3. 如果初始化失败，检查：
   - I2C上拉电阻是否正确连接
   - 硬件连接是否正确
   - I2C地址是否正确（DS3231默认地址�?x68�?
### 测试2：时间读取测�?
1. 程序会自动读取并显示当前时间
2. 观察串口输出的时间格式是否正�?3. 验证时间是否在合理范围内

### 测试3：时间设置测�?
1. 程序会设置时间为2024-01-01 12:00:00
2. 观察串口输出，验证设置是否成�?3. 等待几秒后再次读取，验证时间是否在递增

### 测试4：温度读取测�?
1. 程序会自动读取温�?2. 观察串口输出的温度�?3. 温度应该在合理范围内（通常20-40°C�?
### 测试5：闹钟测�?
1. 程序会配置闹�?为每分钟的第30秒触�?2. 等待到第30秒，观察是否触发
3. 验证闹钟标志是否正确设置和清�?
### 测试6：方波输出测�?
...
├── main_example.c      # 主程序文�?├── board.h             # 硬件配置（I2C2配置�?├── config.h            # 模块开关配�?└── README.md           # 本文�?```
...
1. �?**时间管理**：精确的时间读取和设�?2. �?**温度监控**：内置温度传感器
3. �?**闹钟功能**：两个可编程闹钟
4. �?**方波输出**：可配置频率的方波信�?5. �?**低功�?*：支持电池备�?
...
1. **上拉电阻**�?   - DS3231的SCL和SDA线必须连接上拉电阻（4.7kΩ-10kΩ）到3.3V
   - 这是I2C总线的基本要求，否则通信会失�?
2. **I2C地址**�?   - DS3231的I2C地址固定�?x68�?位地址�?   - 确保没有其他设备使用相同地址

3. **OSF标志**�?   - 如果OSF标志�?，说明振荡器曾经停止，时间可能不准确
   - 需要清除OSF标志并重新设置时�?
4. **硬件I2C限制**�?   - 硬件I2C引脚固定（I2C2: PB10/11�?   - 如果需要灵活引脚配置，使用软件I2C

### 🔑 关键�?
1. **硬件I2C vs 软件I2C**�?   - 硬件I2C：速度快（可达400kHz），占用硬件外设，引脚固�?   - 软件I2C：灵活，可配置任意GPIO引脚，但速度较慢
...
3. **时间格式**�?   - DS3231使用BCD格式存储时间
   - 需要转换为标准格式进行显示和计�?
### 💡 调试技�?
1. **I2C通信失败排查**�?   - 检查上拉电阻是否正确连�?   - 检查引脚配置是否正�?   - 检查I2C地址是否正确�?x68�?   - 使用示波器检查I2C信号
...
6. **硬件I2C与软件I2C性能对比**：硬件I2C和软件I2C在性能上有何差异？在什么场景下应该选择硬件I2C，什么场景下选择软件I2C？如何评估和选择�?   - 使用方波输出作为系统时钟�?   - 实现定时功能

5. **优化显示**�?   - 改进显示格式
   - 添加更多信息显示（星期、日期等�?
6. **对比软件I2C**�?   - 参考I2C01案例，对比硬件I2C和软件I2C的性能差异
   - 理解两种接口的适用场景
