/**
 * @file board.h
 * @brief 硬件配置头文件（I2C03独立工程专用）
 * @details 此文件包含I2C03所需的硬件配置
 * 
...
/* OLED I2C配置 - I2C03配置（软件I2C，PB8/9） */
...
typedef struct
{
    uint8_t line;           /**< EXTI线号（0-19） */
    GPIO_TypeDef *port;     /**< GPIO端口（仅EXTI0-15需要） */
    uint16_t pin;           /**< GPIO引脚（仅EXTI0-15需要） */
    uint8_t trigger;        /**< 触发模式：0=上升沿，1=下降沿，2=双边沿 */
    uint8_t mode;           /**< 模式：0=中断，1=事件 */
    uint8_t enabled;        /**< 使能标志：1=启用，0=禁用 */
} EXTI_Config_t;

/* EXTI统一配置表 - I2C03配置（EXTI0，PA0，下降沿触发，用于DS3231闹钟中断） */
#define EXTI_CONFIGS {                                                                                    \
    {0, GPIOA, GPIO_Pin_0, 1, 0, 1}, /* EXTI0：PA0，下降沿，中断模式，启用（DS3231 INT/SQW引脚） */ \
}
...
/* DMA统一配置表 - I2C03配置（默认禁用所有DMA通道） */
/* 注意：STM32F103C8T6是MD型号，只有DMA1的7个通道，不支持DMA2 */
/* 注意：数组长度必须与DMA_CHANNEL_MAX匹配（MD型号为7） */
#define DMA_CONFIGS {                                                                                    \
    {0, DMA_DIR_PeripheralSRC, DMA_PeripheralInc_Disable, DMA_MemoryInc_Enable, DMA_PeripheralDataSize_Byte, DMA_Mode_Normal, DMA_Priority_Medium, 0}, /* DMA1_CH1：禁用 */ \
    {0, DMA_DIR_PeripheralSRC, DMA_PeripheralInc_Disable, DMA_MemoryInc_Enable, DMA_PeripheralDataSize_Byte, DMA_Mode_Normal, DMA_Priority_Medium, 0}, /* DMA1_CH2：禁用 */ \
    {0, DMA_DIR_PeripheralSRC, DMA_PeripheralInc_Disable, DMA_MemoryInc_Enable, DMA_PeripheralDataSize_Byte, DMA_Mode_Normal, DMA_Priority_Medium, 0}, /* DMA1_CH3：禁用 */ \
    {0, DMA_DIR_PeripheralSRC, DMA_PeripheralInc_Disable, DMA_MemoryInc_Enable, DMA_PeripheralDataSize_Byte, DMA_Mode_Normal, DMA_Priority_Medium, 0}, /* DMA1_CH4：禁用 */ \
    {0, DMA_DIR_PeripheralSRC, DMA_PeripheralInc_Disable, DMA_MemoryInc_Enable, DMA_PeripheralDataSize_Byte, DMA_Mode_Normal, DMA_Priority_Medium, 0}, /* DMA1_CH5：禁用 */ \
    {0, DMA_DIR_PeripheralSRC, DMA_PeripheralInc_Disable, DMA_MemoryInc_Enable, DMA_PeripheralDataSize_Byte, DMA_Mode_Normal, DMA_Priority_Medium, 0}, /* DMA1_CH6：禁用 */ \
    {0, DMA_DIR_PeripheralSRC, DMA_PeripheralInc_Disable, DMA_MemoryInc_Enable, DMA_PeripheralDataSize_Byte, DMA_Mode_Normal, DMA_Priority_Medium, 0}  /* DMA1_CH7：禁用 */ \
}
