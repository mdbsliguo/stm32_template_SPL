/**
 * @file config.h
 * @brief 案例ADC01专用配置（独立工程）
 * @details 只启用案例ADC01需要的模块，禁用其他模块以减小代码体积
 */
...
/* 驱动层模块 */
#define CONFIG_MODULE_GPIO_ENABLED          1   /**< GPIO模块开关 - 必须（ADC、UART、OLED依赖） */
#define CONFIG_MODULE_UART_ENABLED         1   /**< UART模块开关 - 必须（串口调试，新项目必须） */
#define CONFIG_MODULE_ADC_ENABLED          1   /**< ADC模块开关 - 必须（案例核心功能） */
