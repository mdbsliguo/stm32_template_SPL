/**
 * @file board.h
 * @brief 硬件配置头文件（PWM05_InputCapture_DS3231_Freq32K案例独立工程专用）
 * @details 此文件包含案例所需的硬件配置
 * 
 * 注意：这是独立工程的board.h，包含案例所需的硬件配置
 * 
 * 硬件连接：
 * - DS3231 32K输出 -> PA7 (TIM3_CH2，PWMI输入捕获)
...
/* PWM统一配置表 - PWM05案例：TIM3用于PWMI输入捕获，不用于PWM输出 */
/* 注意：数组索引必须对应PWM_Instance_t枚举：0=TIM1, 1=TIM3, 2=TIM4 */
#define PWM_CONFIGS {                                                                                    \
    {NULL, {{NULL, 0, 0}, {NULL, 0, 0}, {NULL, 0, 0}, {NULL, 0, 0}}, 0}, /* TIM1：未使用，占位 */ \
    {NULL, {{NULL, 0, 0}, {NULL, 0, 0}, {NULL, 0, 0}, {NULL, 0, 0}}, 0}, /* TIM3：用于PWMI输入捕获，不用于PWM，禁用 */ \
