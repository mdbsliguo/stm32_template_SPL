/**
 * @file board.h
 * @brief 硬件配置头文件（Bus02_ModBusRTU_Relay案例专用）
 * @details 此文件包含Bus02案例所需的硬件配置
 * 
...
#define UART_CONFIGS {                                                                                    \
    ...
    {USART2, GPIOA, GPIO_Pin_2, GPIOA, GPIO_Pin_3, 9600, USART_WordLength_8b, USART_StopBits_1, USART_Parity_No, 1}, /* UART2：PA2(TX), PA3(RX)，9600（初始值，程序会自动检测并切换），8N1，启用（ModBusRTU通信） */ \
}
