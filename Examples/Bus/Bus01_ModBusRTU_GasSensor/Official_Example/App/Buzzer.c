...
void Buzzer_Init(void)
{
		...
		RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB,ENABLE);          //选择PB口外设时�?
...
void HappyBirthday(void)
{
		...
		{   
				for(j=0;j<SONG_LONG[i]*80;j++)  //播放各个音符，SONG_LONG 为拍子长�?
				{
						Buzzer_TOGGLE;
						for(k=0;k<SONG_TONE[i]*10;k++);//SONG_TONE 延时表决定了每个音符的频�?
				}
				Delay_ms(10);
				i++;
		}
...
//以下是简谱中每个音调的节�?
//�?”对�?个延时单�?�?”对应两个延时单�?�?”对�?个延时单�?
uc16 JP[]=
{
    4,1,1,4,1,1,2,//每行对应一小节音调的节�?
    2,2,2,2,8,
    4,2,3,1,2,2,
    10,
    4,2,2,4,4,
...
void Disirer(void)
{
		...
		while(f[i]!=0xFF)
		{   
				for(j=0;j<JP[i]*300;j++)  //播放各个音符，JP 为拍子长�?
				{
						Buzzer_TOGGLE;
						for(k=0;k<f[i]*5;k++);//F延时表决定了每个音符的频�?
				}
				Delay_ms(10);
				i++;
		}
