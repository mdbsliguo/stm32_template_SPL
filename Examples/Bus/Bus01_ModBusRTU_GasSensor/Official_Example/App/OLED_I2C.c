...
/**
  * @brief  OLED设置光标位置
  * @param  Y 以左上角为原点，向下方向的坐标，范围�?~7
  * @param  X 以左上角为原点，向右方向的坐标，范围�?~127
  * @retval �?
  */
void OLED_SetCursor(uint8_t Y, uint8_t X)
{
	OLED_WriteCommand(0xB0 | Y);					//设置Y位置
	OLED_WriteCommand(0x10 | ((X & 0xF0) >> 4));	//设置X位置�?�?
	OLED_WriteCommand(0x00 | (X & 0x0F));			//设置X位置�?�?
}
...
/**
  * @brief  OLED显示字符�?
  * @param  Line 起始行位置，范围�?~4
  * @param  Column 起始列位置，范围�?~16
...
/**
  * @brief  OLED显示一个汉�? 16x16
  * @param  Line 行位置，范围�?~4
  * @param  Column 列位置，范围�?~16
  * @param  Num 要显示的一个汉字，范围：ASCII可见字符
  * @retval �?
  */
...
{      	
	...
	for (i = 0; i < 16; i++)
	{
		OLED_WriteData(OLED_HZ16x16[Num][i]);			//显示上半部分内容
	}
	OLED_SetCursor((Line - 1) * 2 + 1, (Column - 1) * 8);	//设置光标位置在下半部�?
	for (i = 0; i < 16; i++)
	{
		OLED_WriteData(OLED_HZ16x16[Num+1][i]);		//显示下半部分内容
	}
}
...
void OLED_Init(void)
{
	...
	OLED_WriteCommand(0xA1);	//设置左右方向�?xA1正常 0xA0左右反置
	
	OLED_WriteCommand(0xC8);	//设置上下方向�?xC8正常 0xC0上下反置
