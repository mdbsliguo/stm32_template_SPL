/**
 * @file main_example.c
 * @brief Net05 - W5500 MQTT Client EXTI 示例（小精灵 F103ZE）
 * @example Examples/Net/Net05_W5500_MQTT_C/main_example.c
 * @details MQTT-C 连接 Broker（默认 192.168.101.101:1883），周期 Publish，Subscribe 收命令
 */
...
#define NET_MQTT_TOPIC_MQTTX_SUB  "Net05/stm32_out"
/* MQTTX 点「发布」：给板子发命令（板子 Subscribe 此主题） */
#define NET_MQTT_TOPIC_MQTTX_PUB  "Net05/stm32_in"
...
static void Net_InitOled(void)
{
    ...
    g_app.oled_ready = 1U;
    Net_OledShowLinePadded(1, "Net05 MQTT");
...
static void Net_OnMqttMessage(const char *topic,
                              ...
                              void *user_data)
{
    char topic_buf[MQTT_CLIENT_TOPIC_MAX];
    char payload_buf[96];
    uint16_t topic_n;
    uint16_t payload_n;
...
{
    (void)ctx;
    (void)MQTT_Client_ForceReconnect();
}
...
static void Net_OnPhyLinkUp(void *ctx)
{
    (void)ctx;
    (void)MQTT_Client_ForceReconnect();
    (void)MQTT_Client_Drain(MQTT_CLIENT_DRAIN_DEFAULT);
}
...
static MQTT_Client_Status_t Net_InitMqtt(void)
{
    ...
    st = MQTT_Client_Init(&mqtt_cfg, Net_OnMqttMessage, NULL);
...
static void Net_ProcessPeriodicPublish(void)
{
    ...
    st = MQTT_Client_Publish(NET_MQTT_PUB_TOPIC, payload, (uint16_t)len, NET_MQTT_QOS);
    if (st != MQTT_CLIENT_OK)
    {
        LOG_WARN("MQTT", "publish fail: %d %s", (int)st, MQTT_Client_GetLastErrorStr());
        return;
    }

    LOG_INFO("MQTT", "TX %s", payload);
    (void)MQTT_Client_Drain(MQTT_CLIENT_DRAIN_PUBLISH);
}

static void Net_ProcessOnce(void)
{
    MQTT_Client_Status_t mst;
    uint8_t mqtt_on;
    ...
    mst = MQTT_Client_Drain(MQTT_CLIENT_DRAIN_DEFAULT);
    if (mst != MQTT_CLIENT_OK)
    {
        if (MQTT_Client_IsSessionReady() == 0U)
        {
            LOG_DEBUG("MQTT", "sync: %s", MQTT_Client_GetLastErrorStr());
        }
    }

    mqtt_on = MQTT_Client_IsSessionReady();
    ...
    W5500_PhyMonitor_SetSocketWatch(&g_phy_mon, W5500_PHY_WATCH_TCP_CONN,
                                    (uint8_t)((MQTT_Client_CanPublish() != 0U) ||
                                              (MQTT_Client_IsSessionReady() != 0U)));
}
...
int main(void)
{
    ...
    LOG_INFO("MAIN", "=== Net05 W5500 MQTT Client EXTI ===");
    ...
    if (Net_InitMqtt() != MQTT_CLIENT_OK)
    {
        Net_FatalBlink();
    }
