...
static void Net_ProcessReceive(const W5500_SocketStatus_t *status)
{
    W5500_Status_t st;
    uint16_t read_len;

    if ((status->events & W5500_SOCK_EVT_RECEIVE) == 0U)
    {
        return;
    }
    ...
    LOG_INFO("NET", "RX %u bytes, echo back", (unsigned int)read_len);
