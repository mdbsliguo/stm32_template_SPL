...
static void Pump_ShowStatusLine(uint8_t comm_ok, uint8_t ic_state)
{
    char buffer[20];

    if (comm_ok) {
        snprintf(buffer, sizeof(buffer), "485:OK St:%u    ", (unsigned int)ic_state);
    } else {
        snprintf(buffer, sizeof(buffer), "485:ERR St:%u   ", (unsigned int)ic_state);
    }
...
static void Pump_RefreshOLED(uint32_t delta_1s)
{
    ...
    (void)OGM_FlowIC_GetState(&ic_state);
    (void)OGM_FlowIC_GetInstantFlow(&flow_lpm);
    (void)flow_lpm;
    Pump_ShowStatusLine(g_comm_ok, ic_state);
}
