/**
 * @file board.h
 * @brief 硬件配置头文件（NPN02_OGM 独立工程专用）
 * @details 包含本案例所需的硬件配置（OGM 双 NPN 脉冲、LED、OLED/I2C）
 */
...
/* LED 配置表 - PB12 作为状态指示灯（PA0/PA1 专用于脉冲） */
#define LED_CONFIGS {                                                      \
    {GPIOB, GPIO_Pin_12, Bit_RESET, 1}, /* LED1：PB12，低电平点亮，启用 */ \
}
...
/* 通道 A：PA0（EXTI0）；通道 B：PA1（EXTI1），双边沿触发 */
#define EXTI_CONFIGS {                                                                                            \
    {EXTI_LINE_0, GPIOA, GPIO_Pin_0, EXTI_TRIGGER_RISING_FALLING, EXTI_MODE_INTERRUPT, 1}, /* EXTI0：PA0 通道A */ \
    {EXTI_LINE_1, GPIOA, GPIO_Pin_1, EXTI_TRIGGER_RISING_FALLING, EXTI_MODE_INTERRUPT, 1}, /* EXTI1：PA1 通道B */ \
}
