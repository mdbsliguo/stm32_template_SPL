...
/**
 * @brief ADC初始化
 * @param[in] instance ADC实例索引（ADC_INSTANCE_1）
 ...
 * @param[in] instance ADC实例索引
 * @param[in] channels 通道数组
 * @param[in] channel_count 通道数量
 * @param[out] values 转换结果数组
 * @param[in] timeout 超时时间（毫秒），0表示使用默认超时
 * @return ADC_Status_t 错误码
 */
...
typedef enum {
    ADC_IT_EOC = 0,   /**< 转换完成中断 */
    ADC_IT_JEOC = 1,  /**< 注入转换完成中断 */
    ADC_IT_AWD = 2,   /**< 模拟看门狗中断 */
} ADC_IT_t;
