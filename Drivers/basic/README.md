...
2. **双模式支持**：
   - **GPIO模式**：简单开关控制，无频率控制，适用于有源蜂鸣器
   - **PWM模式**：通过PWM控制频率，支持音调播放，适用于无源蜂鸣器
...
#define BUZZER_CONFIGS {                                                                                    \
    {BUZZER_MODE_GPIO, GPIOA, GPIO_Pin_2, 1, 0, Bit_RESET, 1}, /* Buzzer1：GPIO模式，PA2，低电平有效，启用（PWM实例和通道在GPIO模式下忽略） */ \
}
```
