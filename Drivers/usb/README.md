...
## �?功能特�?
- **USB 2.0全速设�?*：支�?2Mbps全速USB通信
- **端点配置**：支持最�?个端点（EP0-EP7）的配置
- **中断处理**：提供USB中断处理框架
- **事件回调**：支持USB事件回调函数
- **条件编译**：仅MD/HD/CL/XL型号支持，需要外�?8MHz晶振

## ⚠️ 重要说明

1. **硬件要求**�?   - 仅MD/HD/CL/XL型号支持USB
   - 需要外�?8MHz晶振或通过PLL生成48MHz时钟
   - USB引脚：PA11（D-）、PA12（D+�?
2. **协议栈扩�?*�?   - 本模块只提供基础框架
   - 完整的USB协议栈需要根据具体应用扩展：
     - **HID**：人机接口设备（键盘、鼠标等�?     - **CDC**：通信设备类（虚拟串口�?     - **MSC**：大容量存储设备（U盘）
     - **自定义设备类**

3. **端点配置**�?   - EP0：控制端点（必须�?   - EP1-EP7：数据端点（可选）

## 🚀 如何使用

### 1. 配置模块开�?
�?`System/config.h` 中启用USB模块�?
```c
...
- `USB_Status_t USB_Send(uint8_t endpoint, const uint8_t *data, uint16_t length)` - 发送数�?- `USB_Status_t USB_Receive(uint8_t endpoint, uint8_t *data, uint16_t *length)` - 接收数据
...
- `uint8_t USB_IsConnected(void)` - 检查USB是否已连�?- `uint8_t USB_GetStatus(void)` - 获取USB当前状�?
## 🔧 扩展建议

1. **实现USB描述�?*�?   - 设备描述�?   - 配置描述�?   - 接口描述�?   - 端点描述�?   - 字符串描述符

2. **实现标准请求处理**�?   - GetDescriptor
   - SetAddress
   - SetConfiguration
   - GetStatus
   - ClearFeature
...
3. **实现设备类特定功�?*�?   - HID报告描述符和处理
   - CDC接口配置
   - MSC块设备操�?
## ⚠️ 注意事项

- **时钟配置**：确保系统时钟配置正确，USB需�?8MHz时钟
- **中断优先�?*：USB中断优先级应设置合理，避免影响实时�?- **缓冲区管�?*：USB数据缓冲区需要正确管理，避免数据丢失
- **协议栈实�?*：完整的USB协议栈实现需要参考USB 2.0规范和STM32 USB应用笔记
