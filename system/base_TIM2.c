...
/* 模块状态 */
static bool g_base_timer_initialized = false;  /**< 基时定时器初始化标志 */
 ...
 */
static uint8_t BaseTimer_CalculateParams(uint32_t tim2_clk, uint16_t *psc, uint16_t *arr)
{
    /* 目标：1ms中断，即1000Hz */
 ...
 */
static uint32_t BaseTimer_GetTIM2Clock(void)
{
    uint32_t apb1_clk;
    uint32_t tim2_clk;
...
void BaseTimer_Init(void)
{
    ...
    /* 获取TIM2时钟频率 */
    tim2_clk = BaseTimer_GetTIM2Clock();
    
    /* 计算PSC和ARR */
    if (BaseTimer_CalculateParams(tim2_clk, &psc, &arr) != 0)
    {
        /* 配置失败，进入死循环 */
        while (1)
            ;
...
{
    return g_base_timer_initialized;
}
