...
- **LED**：配置表驱动LED控制
- **OLED_SSD1306**：SSD1306驱动，支持软硬I2C，字体/字符/数字显示
- **硬件I2C/软件I2C**：I2C1/I2C2主模式，7位/10位地址，总线扫描
- **硬件SPI/软件SPI**：支持多种SPI模式，8/16位数据格式
...
- [项目架构文档](Docs/PROJECT_ARCHITECTURE.md) - 项目整体架构设计说明
- [项目关系文档](Docs/PROJECT_RELATIONSHIPS.md) - 模块间关系说明
...
- **返回类型**：所有模块函数必须返回`error_code_t`，禁止返回`void`
- **错误处理**：统一错误码系统，参数校验，防御性编程
- **命名规范**：函数名格式为`模块名_功能名`，如`LED_On()`
- **配置驱动**：硬件配置通过`BSP/board.h`统一管理
